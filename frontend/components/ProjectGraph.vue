  ...
  <main class="app-shell">
    <header class="toolbar">
      ...
      <div class="toolbar-actions">
        <span class="graph-count">{{ graph.nodes.length }} nodes · {{ graph.edges.length }} connections</span>
        <button :disabled="loading" @click="loadGraph">{{ loading ? 'Loading…' : 'Refresh' }}</button>
      </div>
    </header>
    ...
    <section class="graph-canvas" aria-label="Project structure graph" :aria-busy="loading">
      ...
      <div v-else-if="!graph.nodes.length" class="notice" role="status">The backend returned an empty project.</div>
