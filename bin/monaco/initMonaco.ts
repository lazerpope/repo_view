...
export function initMonaco(source: Ref<string>): MonacoEditorController {
        ...
        return monaco.languages.registerCompletionItemProvider(LANGUAGE_ID, {
            ...
            provideCompletionItems(currentModel, position) {
                ...
                if (/\bMath\.[A-Za-z_]*$/.test(linePrefix)) {
                    ...
                    const structFields =
                        schemas.get(instanceClasses.get(accessedName ?? '') ?? '') ?? []
                    templates = structFields.map((field) => ({
                        label: field.name,
                        kind: monaco.languages.CompletionItemKind.Property,
                        detail: field.isArray
                            ? 'Static struct array field'
                            : 'Static struct scalar field',
                    ...
                    const declaredCompletions: CompletionTemplate[] = [...declaredNames].map(
                        (label) => ({
                            label,
    ...
    onMounted(() => {
        ...
        editor.onDidChangeModelContent(() => {
            ...
            if (editorValue !== undefined && source.value !== editorValue)
